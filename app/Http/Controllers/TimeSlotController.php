    ...
    public function obtenerFranjasPorFecha($profile_id, $fecha, $retardo = 60)
{
    ...
    try {
        ...
        $fechaActual = now()->setTimezone('America/Bogota')->format('Y-m-d');
        $horaActual = now()->setTimezone('America/Bogota');
        
        // Inicializa la consulta de franjas horarias
        $timeSlotsQuery = TimeSlot::where('day_id', $day->id)
            ->where('available', true); // Solo franjas disponibles
    
        // Si la fecha ingresada es igual a la fecha actual, aplica el filtro del retardo
        if ($fecha === $fechaActual) {
            $hora = $horaActual->copy()->addMinutes($retardo);
            $timeSlotsQuery->where('hour_start', '>=', $hora->format('H:i:s'));
        }
    
        // Obtén las franjas horarias
        $timeSlots = $timeSlotsQuery->get(['hour_start', 'hour_end', 'available']);
